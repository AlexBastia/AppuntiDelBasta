...
\thm{Schwarz}{
  Sia $ f:\mathbb{R}^n\to\mathbb{R} $ tale che $ f, \frac{\partial f}{\partial x_j}, \frac{\partial f}{\partial x_j \partial x_k} $ sono continue in $ \mathbb{R}^n $ $ \forall k,j \in \{1,...,n\} $, allora:
  \[
...
Nella parte quadratica della formula di Taylor in $ \mathbb{R}^n $ compare una "forma quadratica" che dobbiamo studiare per trovare massimi/minimi:
\dfn{Forma quadratica}{
  Sia $ A \in M_n(\mathbb{R}) $ simmetrica ($ A = A^T $), definiamo la funzione $ q_A: \mathbb{R}^n\to\mathbb{R} $:
  \[
...
\dfn{}{
  ...
  \item $ A $ e' indefinita se $ \exists h', h'' \in \mathbb{R}^n. \innerproduct{Ah'}{h'} < 0 < \innerproduct{Ah''}{h''} $
  \end{itemize}
}
\nt{
  Tutte le disuguaglianze sono strette
}
\subsection{Classificazione forme quadratiche (n=2)}
\mprop{}{
  Sia $ A = \begin{pmatrix}
  a & b\\
  b & c\\
...
\pf{Dimostrazione del primo punto}{
  Si calcola la forma quadratica
}
\section{Forma quadratica Hessiana}
