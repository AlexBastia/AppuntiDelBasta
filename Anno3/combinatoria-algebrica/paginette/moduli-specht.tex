% \begin{document}
\chapter{Classificazione delle Rappresentazioni Irriducibili di $\mathfrak{S}_n$}
L'idea e' quella di usare polinomi simmetrici (modo vecchio) e \textit{tableau} (piu' moderno) per poter descrivere le rappresentazioni irriducibili di $ \mathfrak{S}_n $ (e quindi il loro carattere) $ \forall n \in \mathbb{N} $.

\section{Preliminari combinatori}

\subsection{Partizioni e ordini}
...
\subsubsection{Relazioni d'ordine sulle partizioni}
...
\subsection{Tableau di Young}

\subsubsection{Definizioni: SSYT, tableau iniettivo, SYT}
...
\subsubsection{Azione naturale di $\mathfrak{S}_n$ e gruppi di riga/colonna}

L'azione naturale di $\mathfrak{S}_n$ sui tableau iniettivi di forma $\lambda$ è data da: per ogni $\sigma \in \mathfrak{S}_n$, il tableau $\sigma \cdot T$ si ottiene sostituendo ogni entrata $i$ di $T$ con $\sigma(i)$.

\ex{Azione naturale di $ \mathfrak{S}_n $ sui tableau}{
  \begin{center}
    $ (1,3) $
    \begin{ytableau}
    1 \\
...
\begin{itemize}
\item Il \textbf{gruppo di riga} $R(T) \leq \mathfrak{S}_n$ è il sottogruppo delle permutazioni che spostano le entrate di $T$ solo all'interno delle rispettive righe. Si ha $R(T) \cong \mathfrak{S}_{\lambda} := \mathfrak{S}_{\lambda_1} \times \cdots \times \mathfrak{S}_{\lambda_k}$.
\item Il \textbf{gruppo di colonna} $C(T) \leq \mathfrak{S}_n$ è il sottogruppo delle permutazioni che spostano le entrate di $T$ solo all'interno delle rispettive colonne. Si ha $C(T) \cong \mathfrak{S}_{\lambda'}$, dove $\lambda'$ è la partizione coniugata.
\end{itemize}
}
...
\ex{Gruppi di riga e colonna}{
  ...
  \[
    R(T) = \{g \in \mathfrak{S}_5 | g: \{123\} \to \{123\} \lor \{45\} \to \{45\}\} \cong \mathfrak{S}_3 \times \mathfrak{S}_2  
  \]
  e 
  \[
    C(T) = \{g \in \mathfrak{S}_n | g: \{41\} \to \{41\} \lor \{52\} \to \{52\}\} \cong \mathfrak{S}_2 \times \mathfrak{S}_2
  \]
}

\subsubsection{Proprietà di coniugio}
...
\subsection{Lemma di partizione dei tableau}
...
\section{Costruzione del modulo di Specht}

\subsection{Tabloidi e la rappresentazione $M^\lambda$}

\dfn{Tabloide}{\label{def:tabloide}
Il \textbf{tabloide} $\{T\}$ associato a un tableau iniettivo $T$ è l'orbita di $T$ sotto l'azione di $R(T)$. L'ordine degli elementi all'interno di ciascuna riga non ha importanza. L'azione di $\mathfrak{S}_n$ sui tabloidi è $\sigma \cdot \{T\} := \{\sigma \cdot T\}$.
}
...
\dfn{Rappresentazione $M^\lambda$}{\label{def:M-lambda}
Lo spazio vettoriale complesso $M^\lambda$ ha come base formale l'insieme di tutti i tabloidi di forma $\lambda \vdash n$, con l'azione di $\mathfrak{S}_n$ data da permutazione dei tabloidi. Si ha l'isomorfismo
\[
M^\lambda \cong \mathrm{Ind}_{\mathfrak{S}_\lambda}^{\mathfrak{S}_n} \mathbf{1}.
\]
}
...
\subsection{Simmetrizzatori di Young}

\dfn{Simmetrizzatori}{\label{def:simmetrizzatori}
...
\subsection{Vettore di Specht e modulo di Specht}

\subsubsection{Vettore di Specht}

\dfn{Vettore di Specht}{\label{def:vettore-specht}
...
\item Per ogni $\sigma \in \mathfrak{S}_n$: $\sigma \cdot v_T = v_{\sigma \cdot T}$.
\end{enumerate}
}
...
\subsubsection{Modulo di Specht}

\dfn{Modulo di Specht}{\label{def:modulo-specht}
...
\subsection{Teorema della base standard (enunciato)}

\thm{Base standard dei moduli di Specht}{\label{thm:base-standard-specht}
...
\ex{Modulo di Specht $S^{(2,1)}$ per $\mathfrak{S}_3$}{
...
\]
I vettori $v_T, v_{T'}$ sono linearmente indipendenti e formano la base standard di $S^{(2,1)}$, che è la rappresentazione standard irriducibile di $\mathfrak{S}_3$ ($\dim S^{(2,1)} = 2$), mentre $ S^{(3)} $ è la rappresentazione banale (triviale) e $ S^{(1^3)} $ è la rappresentazione segno.
}
...
\ex{}{
  ...
  Quindi $\mathrm{sh}(\{T\}) = \big((0,0),\, (1,0),\, (2,0),\, (2,1),\, (2,2)\big)$.
}
...
\pf{Dimostrazione del Teorema (Base Standard)}{
  Sappiamo che $|\mathfrak{S}_n| = n! = \sum_{\lambda \vdash n} (\dim S^\lambda)^2$ (dalla decomposizione della rappresentazione regolare e dal teorema di Maschke/Wedderburn: $\mathbb{C}[\mathfrak{S}_n] \cong \bigoplus_\lambda \mathrm{End}(S^\lambda)$, prendendo le dimensioni). Dall'indipendenza lineare si ha $\dim S^\lambda \geq f^\lambda$, quindi:
  \[
...
\section{Classificazione delle rappresentazioni irriducibili di $\mathfrak{S}_n$}

\subsection{Il Lemma Tecnico}
...
\thm{Classificazione delle rappresentazioni irriducibili di $\mathfrak{S}_n$}{\label{thm:classificazione-specht}
I moduli di Specht $\{S^\lambda \mid \lambda \in \mathrm{Par}(n)\}$ costituiscono un sistema completo di rappresentazioni irriducibili di $\mathfrak{S}_n$, a due a due non isomorfe.
}
...
\item \textbf{Completezza.} La famiglia $\{S^\lambda\}_{\lambda \vdash n}$ è indicizzata dalle partizioni di $n$, che sono in biiezione con le classi di coniugio di $\mathfrak{S}_n$. Poiché il numero di rappresentazioni irriducibili di un gruppo finito coincide con il numero delle sue classi di coniugio, la famiglia è completa.
\end{enumerate}
}
