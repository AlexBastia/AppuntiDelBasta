...
\pf{Dimostrazione}{
  Supponiamo per assurdo che $\sum_{T \in \mathrm{SYT}(\lambda)} \gamma_T\, v_T = 0$ con $\gamma_T \in \mathbb{C}$ non tutti nulli. Tra tutti i tableau standard con $\gamma_T \neq 0$, scegliamo $\overline{T}$ tale che il tabloide $\{\overline{T}\}$ sia \textbf{massimale} nell'ordine parziale sui tabloidi.
  ...
  \]
  \textit{solo} con il termine $q = \mathrm{id}$ (coefficiente $+1$). Infatti, per ogni $q \in C(\overline{T})$ con $q \neq \mathrm{id}$, il Lemma garantisce $\{\overline{T}\} \not\leq \{q\overline{T}\}$, quindi i tabloidi $\{q\overline{T}\}$ sono tutti diversi da $\{\overline{T}\}$.

  Affinche' la somma totale faccia zero, il termine $\{\overline{T}\}$ deve essere cancellato dall'espansione di un altro vettore $v_{\widetilde{T}}$ con $\gamma_{\widetilde{T}} \neq 0$. Poiche' tableau standard distinti producono tabloidi distinti, $\{\overline{T}\}$ puo' emergere da $v_{\widetilde{T}}$ solo come termine permutato, cioe' deve valere $\{\overline{T}\} = \{\widetilde{q}\widetilde{T}\}$ per qualche $\widetilde{q} \in C(\widetilde{T})$ con $\widetilde{q} \neq \mathrm{id}$.

  Ma applicando il Lemma a $\widetilde{T}$, si ha $\{\widetilde{T}\} \not\leq \{\widetilde{q}\widetilde{T}\} = \{\overline{T}\}$, cioe' $\{\widetilde{T}\} \not\leq \{\overline{T}\}$. Questo contraddice la massimalita' di $\{\overline{T}\}$, perche' $\widetilde{T}$ ha coefficiente non nullo ma il suo tabloide non e' dominato da $\{\overline{T}\}$.

  Quindi nessun altro vettore puo' cancellare il termine $\{\overline{T}\}$: nella combinazione lineare $\sum \gamma_T v_T = 0$, il tabloide $\{\overline{T}\}$ compare con coefficiente $\gamma_{\overline{T}} \neq 0$. Ma i tabloidi sono base di $M^\lambda$, quindi linearmente indipendenti: contraddizione.
}
...
\pf{Dimostrazione del Teorema (Base Standard)}{
  Sappiamo che $|S_n| = n! = \sum_{\lambda \vdash n} (\dim S^\lambda)^2$ (formula di Burnside, dato che gli $S^\lambda$ sono le irriducibili di $S_n$). Dall'indipendenza lineare si ha $\dim S^\lambda \geq f^\lambda$, quindi:
  \[
