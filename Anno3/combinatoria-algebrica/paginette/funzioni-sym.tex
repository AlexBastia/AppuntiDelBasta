...
\pf{Dim}{
  \textbf{Condizione di dominanza.} Supponiamo $ M_{\lambda,\mu} \neq 0 $, cosi' per la
  proposizione precedente esiste una matrice $ 0$-$1 $ $ A $ con $ row(A) = \lambda $ e
  $ col(A) = \mu $. Sia $ A' = (a'_{i,j})_{i,j \geq 1} $ la matrice $ 0$-$1 $ con
  $ row(A') = \lambda $ e gli $ 1 $ di ogni riga giustificati a sinistra, cioe'
  $ a'_{i,j} = 1 \iff 1 \leq j \leq \lambda_i $. Per ogni $ i $, il numero di $ 1 $
  nelle prime $ i $ colonne di $ A' $ non e' inferiore a quello in $ A $: per
  definizione di ordine di dominanza, $ col(A') \geq col(A) = \mu $. Ma
  $ col(A') = \lambda' $, dunque $ \lambda' \geq \mu $.
