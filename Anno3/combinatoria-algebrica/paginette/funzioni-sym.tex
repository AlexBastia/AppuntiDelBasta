...
\dfn{Funzione simmetrica monomiale}{
  Data una partizione $ \lambda \vdash n $, la \textit{funzione simmetrica monomiale} e':
  \[
    m_\lambda(x) := \sum_{\substack{\alpha \models n \\ \lambda(\alpha) = \lambda}} x^\alpha
  \]
  dove $ x^\alpha = x_1^{\alpha_1} x_2^{\alpha_2} \cdots x_k^{\alpha_k} $ e la somma varia su tutte le
  composizioni deboli $ \alpha $ di $ n $ tali che, riordinate e private degli zeri, restituiscono
  esattamente $ \lambda $.

  In altri termini, $ m_\lambda(x) $ e' la somma di tutti i monomi \textit{distinti} ottenibili
  permutando gli esponenti di $ \lambda $ sulle variabili $ x_1, \ldots, x_k $: e' la somma
  dell'orbita di $ x^\lambda $ sotto l'azione di $ S_k $.
}

\ex{$ \lambda = (2,1,1) $, $ k = 3 $}{
  Gli esponenti da distribuire sono $ 2, 1, 1 $. Le composizioni deboli di 4 in 3 parti con
  $ \lambda(\alpha) = (2,1,1) $ sono $ (2,1,1) $, $ (1,2,1) $, $ (1,1,2) $, quindi:
  \[
    m_{(2,1,1)}(x_1, x_2, x_3) = x_1^2 x_2 x_3 + x_1 x_2^2 x_3 + x_1 x_2 x_3^2
  \]
  E' un polinomio simmetrico: $ m_\lambda \in \mathrm{Sym}^n[X_k] $.
}

\thm{Le $ m_\lambda $ sono una base di $ \mathrm{Sym}^n[X_k] $}{
  L'insieme $ \{m_\lambda \mid \lambda \vdash n,\ \ell(\lambda) \leq k\} $ e' una base di
  $ \mathrm{Sym}^n[X_k] $. In particolare, per $ k \geq n $:
  \[
    \dim \mathrm{Sym}^n[X_k] = p(n) = \#\mathrm{Par}(n).
  \]
}
\pf{Dim}{
  Dobbiamo mostrare che le $ m_\lambda $ generano $ \mathrm{Sym}^n[X_k] $ e sono linearmente
  indipendenti.

  \textbf{Generazione.}
  Un generico $ f \in \mathrm{Sym}^n[X_k] $ e' una combinazione lineare di monomi:
  \[
    f = \sum_{\alpha \models n} c_\alpha\, x^\alpha.
  \]
  L'invarianza $ \sigma \cdot f = f $ per ogni $ \sigma \in S_k $ forza tutti i monomi della
  stessa orbita ad avere lo stesso coefficiente: se $ \lambda(\alpha) = \lambda(\beta) $
  (cioe' gli esponenti di $ \alpha $ e $ \beta $ sono una permutazione l'uno dell'altro),
  allora $ c_\alpha = c_\beta =: c_\lambda $. Raggruppando per orbita:
  \[
    f = \sum_{\lambda \vdash n,\, \ell(\lambda) \leq k} c_\lambda
        \underbrace{\Bigl(\sum_{\substack{\alpha \models n \\ \lambda(\alpha)=\lambda}} x^\alpha\Bigr)}_{= m_\lambda(x)}
    = \sum_\lambda c_\lambda\, m_\lambda.
  \]

  \textbf{Indipendenza lineare.}
  Per costruzione, per partizioni distinte $ \lambda \neq \mu $ i monomi che compongono
  $ m_\lambda $ e quelli di $ m_\mu $ appartengono a orbite disgiunte, quindi non si cancellano.
  Pertanto nessuna combinazione lineare non banale delle $ m_\lambda $ puo' annullarsi.
}
\ex{$ n = 2 $, $ k = 3 $}{
  $ f = 3x_1x_2 + 3x_2x_3 + 3x_1x_3 $. I tre monomi hanno $ \lambda(\alpha) = (1,1) $, dunque
  lo stesso coefficiente $ c_{(1,1)} = 3 $, e si raggruppano in $ f = 3\, m_{(1,1)} $.
}

\dfn{Notazioni matriciali}{
  Sia $ A = (a_{ij})_{i,j \geq 1} $ una matrice con un numero finito di entrate non nulle. Si pone:
  \[
    r_i(A) = \sum_j a_{ij}, \qquad c_j(A) = \sum_i a_{ij}
  \]
  (somme di riga e di colonna). Definiamo $ row(A) = (r_1, r_2, \ldots) $ e
  $ col(A) = (c_1, c_2, \ldots) $. Una matrice $ 0$-$1 $ ha coefficienti in $ \{0,1\} $.
}

\dfn{Funzione simmetrica elementare}{
  Per ogni $ s \geq 0 $:
  \[
    e_s := m_{(1^s)} = \sum_{i_1 < i_2 < \cdots < i_s} x_{i_1} x_{i_2} \cdots x_{i_s}.
  \]
  Per convenzione $ e_0 = 1 $. E' la somma di tutti i prodotti di $ s $ variabili
  \textit{distinte}. Data $ \lambda = (\lambda_1, \ldots, \lambda_k) \vdash n $:
  \[
    e_\lambda := e_{\lambda_1} \cdot e_{\lambda_2} \cdots e_{\lambda_k}.
  \]
}
...
\ex{$ k = 3 $, $ \lambda = (2,2,1) $}{
  ...
  \]
  Per $ \lambda = (2,2,1) $: $ e_{(2,2,1)} = e_2 \cdot e_2 \cdot e_1 = e_2^2 \cdot e_1 $
  (prodotto, non somma).
}

\mprop{Sviluppo di $ e_\lambda $ nella base monomiale}{
  Per ogni $ \lambda \vdash n $:
  \[
    e_\lambda = \sum_{\mu \vdash n} M_{\lambda\mu}\, m_\mu
  \]
  dove $ M_{\lambda\mu} $ e' il numero di matrici $ 0$-$1 $ con $ row(A) = \lambda $ e
  $ col(A) $ avente partizione associata $ \mu $.
}
\pf{Idea}{
  Sviluppando $ e_\lambda = e_{\lambda_1} \cdots e_{\lambda_k} $, ogni monomio si ottiene
  scegliendo dal fattore $ e_{\lambda_i} $ un insieme di $ \lambda_i $ variabili distinte.
  Questo corrisponde a una matrice $ 0$-$1 $ con $ row(A) = \lambda $: la riga $ i $ indica
  quali variabili si scelgono in $ e_{\lambda_i} $. Il tipo del monomio risultante e'
  $ \mu = \lambda(col(A)) $.
}

\ex{$ \lambda = (3,1) $, $ n = 4 $}{
  $ e_{(3,1)} = e_3 \cdot e_1 $. Due casi:
  \begin{itemize}
    \item La variabile di $ e_1 $ e' gia' in $ e_3 $: un esponente diventa 2, gli altri due
          restano 1. Tipo $ (2,1,1) $. Si trova $ M_{(3,1),(2,1,1)} = 1 $.
    \item La variabile di $ e_1 $ e' nuova: quattro esponenti pari a 1. Tipo $ (1,1,1,1) $.
          Si trova $ M_{(3,1),(1,1,1,1)} = 4 $.
  \end{itemize}
  Quindi $ e_{(3,1)} = m_{(2,1,1)} + 4\,m_{(1,1,1,1)} $.
}

\thm{$ \{e_\lambda\} $ e' una base di $ \mathrm{Sym}^n[X_k] $}{
  L'insieme $ \{e_\lambda \mid \lambda \vdash n,\ \ell(\lambda') \leq k\} $ e' una base di
  $ \mathrm{Sym}^n[X_k] $.
}
\pf{Dim}{
  \textbf{Condizione di dominanza.} Sia $ M_{\lambda\mu} \neq 0 $, cioe' esiste $ A $ con
  $ row(A) = \lambda $ e $ col(A) \sim \mu $. Costruiamo $ A' $ spostando gli $ 1 $ di ogni
  riga il piu' a sinistra possibile: si ottiene $ col(A') = \lambda' $. Poiche' spostare a
  sinistra non diminuisce le somme parziali di colonna, $ \lambda' \geq \mu $
  nell'ordine di dominanza. Quindi $ M_{\lambda\mu} = 0 $ se $ \mu \not\leq \lambda' $.

  \textbf{Diagonale unitaria.} La matrice $ A' $ con $ row = \lambda $ e $ col = \lambda' $
  e' unica (gli $ 1 $ sono forzati a sinistra in modo determinato), quindi
  $ M_{\lambda\lambda'} = 1 $.

  \textbf{Conclusione.} Rispetto all'ordine lessicografico inverso, $ (M_{\lambda\mu}) $
  e' triangolare superiore con $ 1 $ sulla diagonale, dunque invertibile. Poiche'
  $ \{m_\lambda\} $ e' una base, anche $ \{e_\lambda\} $ lo e'.
}

\ex{$ n = 4 $}{
  Ordine: $ (4) > (3,1) > (2,2) > (2,1,1) > (1^4) $. La matrice:
  \[
...
\section{Funzioni simmetriche omogenee $ h_\lambda $}

\dfn{Funzione simmetrica omogenea completa}{
  Per ogni $ n \geq 0 $, la \textit{funzione simmetrica omogenea completa} e':
  \[
    h_n := \sum_{i_1 \leq i_2 \leq \cdots \leq i_n} x_{i_1} x_{i_2} \cdots x_{i_n}
    = \sum_{\lambda \vdash n} m_\lambda.
  \]
  Per convenzione $ h_0 = 1 $. A differenza di $ e_n $, gli indici possono ripetersi.
  Data $ \lambda \vdash n $:
  \[
    h_\lambda := h_{\lambda_1} \cdot h_{\lambda_2} \cdots h_{\lambda_k}.
  \]
  Anche $ \{h_\lambda \mid \lambda \vdash n\} $ e' una base di $ \mathrm{Sym}^n[X_k] $
  (stesso argomento di triangolarita', usando matrici $ \mathbb{N} $ invece di $ 0$-$1 $).
}

\ex{$ k = 3 $}{
  \[
    h_2 = x_1^2 + x_2^2 + x_3^2 + x_1x_2 + x_1x_3 + x_2x_3 = m_{(2)} + m_{(1,1)}.
  \]
}
...
\thm{$ H(t) \cdot E(-t) = 1 $}{
  ...
  \[
    H(t) := \sum_{n \geq 0} h_n\, t^n = \prod_i \frac{1}{1-x_i t}, \qquad
    E(t) := \sum_{n \geq 0} e_n\, t^n = \prod_i (1 + x_i t)
  \]
  soddisfano $ H(t) \cdot E(-t) = 1 $.
}
\pf{Dim}{
  $ \frac{1}{1-x_i t} = \sum_{m \geq 0} x_i^m t^m $ raccolta per grado da' $ h_n $; i prodotti
  $ \prod_i (1+x_i t) $ raccolgono i monomi con variabili distinte, dando $ e_n $.
  La relazione segue direttamente:
  \[
    H(t) \cdot E(-t) = \prod_i \frac{1}{1-x_it} \cdot \prod_i (1-x_it) = 1.
  \]
}

\cor{Ricorsione tra $ e_n $ e $ h_n $}{
  Per ogni $ n \geq 1 $, estraendo il coefficiente di $ t^n $ in $ H(t) \cdot E(-t) = 1 $:
  \[
...
\ex{$ n = 2 $}{
  $ h_2 - e_1 h_1 + e_2 = 0 $, cioe' $ h_2 = e_1^2 - e_2 $. (Confronta con il risultato
  dalle Identita' di Newton: $ h_2 = \frac{p_1^2 + p_2}{2} $.)
}
