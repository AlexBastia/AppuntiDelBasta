...
\pf{Dim}{
  Per induzione su $ \ell $.

  \textbf{Base ($ \ell = 1 $).} $ A(G)_{ij} $ e' per definizione il numero di lati fra $ v_i $ e $ v_j $, ovvero il numero di cammini di lunghezza $ 1 $.

  \textbf{Passo induttivo.}
  \[
    \bigl(A(G)^{\ell+1}\bigr)_{ij}
      = \bigl(A(G)^\ell \cdot A(G)\bigr)_{ij}
      = \sum_{k=1}^{p} \bigl(A(G)^\ell\bigr)_{ik}\, A(G)_{kj} .
  \]
  Per ipotesi induttiva $ \bigl(A(G)^\ell\bigr)_{ik} $ conta i cammini di lunghezza $ \ell $ da $ v_i $ a $ v_k $, e $ A(G)_{kj} $ il numero di lati da $ v_k $ a $ v_j $. La somma su $ k $ conta tutte le concatenazioni: un cammino di lunghezza $ \ell + 1 $ da $ v_i $ a $ v_j $ si scompone univocamente come (cammino di lunghezza $ \ell $ da $ v_i $ a un intermedio $ v_k $) seguito da (un lato fra $ v_k $ e $ v_j $).
}
...
\pf{Dim}{
  Elevando a potenza $ \ell $ la diagonalizzazione (coniugio = automorfismo, quindi commuta con la potenza):
  \[
    U^T A(G)^\ell\, U = D^\ell = \mathrm{diag}(\lambda_1^\ell, \ldots, \lambda_p^\ell)
    \quad\Longrightarrow\quad A(G)^\ell = U\, D^\ell\, U^T = \sum_{k=1}^{p} \lambda_k^\ell\, u_k u_k^T .
  \]
  L'entrata $ (i,j) $ di $ u_k u_k^T $ e' $ u_{ik}\, u_{jk} $, da cui la formula.
}
