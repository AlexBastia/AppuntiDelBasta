...
\ex{Rare Classical Monte Carlo L}{
  ...
  \lstinputlisting[language=R]{code/Examples/MC_Int/Ex3_4.R}
...
\begin{itemize}
  ...
  \item $ Var(h(X)f(X)/g(X)) < \infty$, which basically means that the tails of $ g $ have to be heavier (like accept-reject)
\end{itemize}
...
\ex{3.4}{
  ...
  \begin{enumerate}
    ...
    \item Try importance sampling with an importance function $ g(x) = Unif(-8, -1) $ with a sample size of $ n = 10^3 $.
  \end{enumerate}

  The importance sampling is surely wrong because the supports aren't contained. The other condition is the harder one to actually check. Note that the weights aren't probabilities. 
}

\ex{Tail Probabilities}{
  We can use importance sampling. Seen as it's very rare to sample a value over 4.5, we can calculate the inverse. We use an exponential distribution which is truncated at $ a $ (starts from a different point). This is usefull as it resembles the tails of distributions. So our domain is $ [a,+\infty[ $, we need to be sure it interates to 1:
  \[
    g(y|y>a) = \frac{g(y, y>a)}{g(y>a)} = \frac{g(y)}{g(y>a)} = \frac{e^{-y}}{1-F(a)}
  \]
  Seen as we have a closed form solution for the CDF of an exponential, we can calculate the value
  \[
    e^{-(y-4.5)}
  \]
  $ h(x) $ is an indicator function.
}

The choice of $ g $ isn't trivial. We want the ratio to be stable and not diverge (to have finite variance). In practice we can compute the weights and check the variance that way (not formal but good aprox).
