...
\nt{
  although an integral it is not usual to appear in this form (for instance $\int_a^bg(x)dx$), we can always rewrite it in this form by choosing a suitable density $f(x)$. For example, if we want to compute $\int_a^bg(x)dx$, we can choose $f(x)$ as the uniform density in $[a,b]$:
  \[
    f(x) = \begin{cases}
      \frac{1}{b-a} & x \in [a,b] \\
      0 & otherwise
...
\subsection{The low of Large Numbers and low of Central limit}

Consider an infinite sequence of independent and identically distributed (i.i.d.) random variables $X_1, X_2, \dots$ defined on a probability space $(\Omega, \mathcal{F}, P)$. Let these variables have a finite expectation $\mathbb{E}[X_1] = \mu$.
...
\thm{Strong Law of Large Numbers (SLLN) - Convergence Almost Surely}{
 ...
 \nt{
  If the random variables also have a finite variance $Var(X_i) = \sigma^2 < \infty$, then the variance of the sample mean decreases linearly with $n$:
  \[
...
\thm{Central Limit Theorem (CLT)}{
  ...
  Then, the random variable $\sqrt{n}(\overline{X}_n - \mu)$ converges in \textit{distribution} to a Normal random variable with mean 0 and variance $\sigma^2$ as $n \to \infty$:
  \[
