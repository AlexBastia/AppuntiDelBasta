...
\dfn{Gradient}{
    The \textit{gradient} of the log-likelihood function $\ell(w)$ (which the slide denotes as $E[w]$) is a vector composed of the partial derivatives of $\ell(w)$ with respect to each parameter $w_i$. It points in the direction of the steepest ascent on the likelihood surface.
    \[
        \nabla_w [\ell] = \left[ \frac{\partial \ell}{\partial w_0}, \frac{\partial \ell}{\partial w_1}, \dots, \frac{\partial \ell}{\partial w_n} \right]
    \]
}
...
\nt{
  This is a \textit{local} technique: it's not given that the minimum we find is global, because it could just be a local one. For logistic regression, we can be certain it's global, but for more complicated models this isn't always true.
...
The concept is conceptually simple. We can visualize the model's error function, $E(w)$, as a topographical map. On this map, the altitude represents the error (the vertical axis), and the goal is to find the lowest point (the minimum error).
