...
\pf{Derivation for Bernoulli}{
    ...
    Therefore, the MLE is simply the observed frequency of successes.
}
...
\nt{Multinomial distribution is $P(X^n = a_i\mid \theta) = c_{a_i} \sum_{i}\theta_{i}^{\alpha_i}$. where $\alpha_i$ is the number of i in the sequence and $ c_{a_i}$ is a
combinatorial constant not depending on $\theta$
