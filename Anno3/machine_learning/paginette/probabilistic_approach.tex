...
\[
l(w) = \sum_\ell \log P(y^\ell | x^\ell, w) = \sum_\ell \left( y^\ell \cdot \log P(Y=1|x^\ell, w) + (1-y^\ell) \cdot \log P(Y=0|x^\ell, w) \right)
\]

Using the logistic function, we can set
\[
  \alpha^\ell = \sigma\left(w_0 + \sum_{i}w_ix_i^\ell\right)
\]
...
\dfn{Gradient}{
  The \textit{gradient} of a function $ f(w_0, ..., w_n) $  is a vector composed of the partial derivatives of $f$ with respect to each parameter $w_i$. It points in the direction of the steepest ascent on the function's surface.
    \[
        \nabla_w [f] = \left[ \frac{\partial f}{\partial w_0}, \frac{\partial f}{\partial w_1}, \dots, \frac{\partial f}{\partial w_n} \right]
    \]
}

We can use the definition of the gradient in order to find the direction that lowers the loss function $ E(w) $ the most (the direction of greatest descent) and to subsequently perform a "step" (determined by the learning rate) in that direction.
This is a \textit{general} technique that can be applied to any differentiable function.

\nt{
  This is also a \textit{local} technique: it's not given that the minimum we find is global, because it could just be a local one. For logistic regression, we can be certain it's global (as we'll see later), but for more complicated models this isn't always true.
...
\thm{Training Rule (Gradient Descent)}{
    To find the parameters $w$ that minimize $E(w)$, we use \textit{Gradient Descent}. We start with an initial guess for $w$ and iteratively update the parameters by taking small steps in the opposite direction of the gradient. The update rule for a single weight $w_i$ is:
    \[
...
\subsection{Error function for Logistic Regression}
...
\thm{Convexity of the Error Surface for Logistic Regression}{
    Let $E(w) = -l(w)$ be the error function for a logistic regression model, defined as the \textit{negative log-likelihood} of the training data.
...
\dfn{Probability that sample $\ell$ is in category $Y = 1$}{
    \begin{equation*}
        P(Y = 1|x^\ell , w ) = \sigma ( w_0+\sum_i w_i x^\ell_i) = \alpha^\ell
    \end{equation*}
}
then:
\dfn{Log-likelihood}{
    \begin{equation}
        l(w) = \sum_\ell \log P(Y = y^\ell | x^\ell, w) = \sum_\ell \left( y^\ell \log(\alpha^\ell) + (1-y^\ell)\log(1-\alpha^\ell) \right)
    \end{equation}
}
...
We want to compute the partial derivative $\frac{\partial l(w)}{\partial w_i}$ of the log-likelihood function $l(w)$.
    First, let's define the components, where $\alpha^\ell = \sigma(z^\ell)$ and $z^\ell = w_0 + \sum_i w_i x_i^\ell$.

    \textit{Step 1: Derivative of $\log(\alpha^\ell)$ w.r.t. $z^\ell$}
    \[
    ...
    \textit{Step 2: Derivative of $\log(1-\alpha^\ell)$ w.r.t. $z^\ell$}
    ...
    \textbf{Note:} on the slide the prof makes the mistake of not parameterizing $ z $ over $ \ell $, leading to a result that doesn't make any sense (What is $ x_i^\ell $ outside of the sum? We don't have a value for $ \ell $!). We can fix this by applying the chain rule inside the sum, leading to a valid value for $ z^\ell $.
}
...
\dfn{Update Rule with Regularization}{
    ...
    \nt{The slide shows a sign flip in the gradient term, which implies a switch from maximizing log-likelihood (Gradient Ascent) to minimizing a negative log-likelihood (Gradient Descent). The logic remains the same: the regularizer always pushes the weight towards zero.}
...
\begin{itemize}
  \item \textbf{Discriminative}: used to learn the \textbf{decision boundary} that separates different classes in the data.
\item \textbf{Generative}: used to model the \textbf{underlying data distribution}.
\end{itemize}

Both of these models can be used for data classification (estimating $ f:X \to Y $ or $ P(Y|X) $), but only generative models are used to generate new data or for data augmentation.

\ex{Generative vs Discriminative classifiers}{
  \textbf{Discriminative} (e.g. Logistic regression)
  \begin{itemize}
    \item Assume a distribution for $ P(Y|X) $ or a shape for the discrimination function
    \item Estimate the parameters for $ P(Y|X) $ (or for the discrimination function) from training data
  \end{itemize}

  \textbf{Generative} (e.g. Naive Bayes)
  \begin{itemize}
    \item Assume a distribution for $ P(X|Y) $ and $ P(X) $
    \item Estimate parameters for $ P(X|Y) $ and $ P(X) $ from training data
    \item Use Bayes' rule to infer $ P(Y|X) $
  \end{itemize}
}

\subsection{Linear models}
Both generative and discriminative models can be \textit{linear}, meaning that features are considered independently from one another.
...
\begin{itemize}
\item Logistic regression: although the function $ \sigma $ is not linear, the way its input varies given the features is.
\item Naive Bayes: as a classifier, it's linear when considering multinomial or bernullian distributions and in some cases with Gaussian distribution.
\end{itemize}
