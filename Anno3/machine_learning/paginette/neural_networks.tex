...
\]
\subsubsection{Dense processing}
...
\begin{itemize}
\item \textbf{Padding}: extra space added around the input
\item \textbf{Stride}: how the kernel moves over the input
\end{itemize}
...
Where
\begin{itemize}
\item W is the spatial dimension of the I/O
...
\begin{itemize}
\item they reduce spatial dimensions
\item they provide some tollerance to translations
\end{itemize}
...
An \textbf{autoencoder} is a network trained to reconstruct input data out of a learned internal representation.

Usually, the internal representation has lower dimensionality w.r.t. the input data. This compression is made possible by exploiting \textbf{regularities} (correlatins) in the features describing the input data.

\nt{
  The higher the \textbf{entropy} of the structure of the input data, the lower the amount of possible compression is.
}

The type of compression is:
\begin{itemize}
\item \textbf{Data-specific}: only works well on data with strong correlations.
\item \textbf{Lossy}: the output is degraded w.r.t. the input.
\item \textbf{Directly trained}: on unlabeled data samples (\textbf{self-supervised} training).
\end{itemize}
...
\section{Generative Models} % 4 Dic - slides7
\dfn{Generative Model}{
...
\begin{itemize}
\item explicitly estimate the distribution
\item build a generator able to sample according to $ p_{\text{model}} $
\end{itemize}
...
where
\begin{itemize}
  \item $ P(z) $ is the \textbf{prior distribution} over latent variables (typically Gaussian)
  \item $ P(X|z) $ is the likelyhood function approximated by the deteministic generator $ G(z) $.
\end{itemize}

Generators decode points in the latent space to produce output. Seen as they're continuous, we can identify \textbf{trajectories} for editing and manipulating the latent representation of the data directly.
...
\begin{itemize}
\item If we start with a real image, we need to determine its corresponding latent encoding in some way.
\item If we start with an encoding, we must decide what image to generate and identify the factors that could drive this generation.
\end{itemize}
...
The generator is coupled with an \textbf{encoder} producing a latent encoding $ z  $ given $ X $ distributed according to an inference distribution $ Q(z|X) $.

The loss function aims to:
\begin{itemize}
  \item minimize the reconstruction error between $ X $ and $ \hat{X} $
  \item bring the marginal inference distribution $ Q(z) $ close to the prior $ P(z) $
\end{itemize}

It's approach is reminiscent of the autoencoder idea.


\subsection{Generative Adversarial Networks} % slides 8 - 5 Dic
The generator is coupled with a \textbf{discriminator} trying to tell apart \textbf{real} data from \textbf{fake} data produced by the generator.

The detector and generator are trained together, the loss function aims to:
\begin{itemize}
\item instruct the detector to spot the generator
\item instruct the generator to fool the detector
\end{itemize}

This behaviour can be formalized as a \textbf{Min-Max game}, where the detector wants to maximize the amount of correct guesses and the generator wants to minimize this amount
\[
  Min_GMax_DV(D,G)
\]
Where
...
\begin{itemize}
\item The negative cross-entropy of the discriminator w.r.t. the true data distribution $ p_{\text{data}} $
\item The negative corss-entropy of the "false" discriminator w.r.t. the fake generator (where $ z $ is a random sample in the latent space)
\end{itemize}
...
\begin{algorithm}[H]
    ...
    repeat\;
      ...
      freeze Generator's weights $ \Theta_G $\;
      sample a batch of real data $ x \sim P_{\text{data}} $\;
      sample a batch of noise $ z \sim N(0,1) $\;
      update the Discriminator's weights $ \Theta_D $ by stochastic ascent on:
      \[
        \nabla_{\Theta_D}[V(D,G)] 
      \]
      // Train the generator G
      freeze Discriminator's weights $ \Theta_D $\;
      sample a batch of noise $ z \sim N(0,1) $\;
      update the Generator's weights $ \Theta_G $ by stochastic ascent on:
      \[
...
\begin{itemize}
\item From left-to-right we have \textbf{forward diffuson}, where data is progressivly corrupted by adding noise at each step.
\item From right-to-left we have \textbf{backward diffusion}, where the model learns to denoise the corrputed immage.
\end{itemize}

Each backward step involves two operations:
\begin{itemize}
\item \textbf{Denoising}: try to remove all the noise in the image.
\item \textbf{Noise reinjection}: add back a small amount of noise
\end{itemize}

\subsection{Denoising}
It's the main and only trainable component of the model. It's used to predict the noise contained in an image.

It's unique for each step $ t $, seen as there's a fixed amount of noise the image should have at each step (the signal rate).

So the network takes in an input:
\begin{itemize}
\item A noisy image $ x_t $
\item A signal rate $ \alpha_t $ that expresses the amount of the original signal left in the data
\end{itemize}
...
\subsection{Noise reinjection}
Can be \textbf{deterministc} (DDIM) or \textbf{probabilistic} (DDPM):
\begin{itemize}
\item DDPM reinjects a random amount of noise each time
\item DDIM reinjects the same noise that was removed but at a lesser intensity, so it's deterministic given the starting noise.
\end{itemize}

\subsection{Training}
Consists of running these steps until converged:
\begin{itemize}
  \item Take a sample (clean image)
  \item Randomly select a timestamp $ t \in [1,...,T] $
  \item Create random Guassian noise $ \epsilon $
  \item Corrupt the sample with the signal rate $ \alpha_t $ and the gaussian noise
  \item Predict the error $ \alpha_{\theta} $
  \item Use backpropagation to better the prediction
\end{itemize}

\subsection{Sampling}
To create a new image from a trained diffusion model:
\begin{itemize}
\item Generate random Gaussian noise
\item Starting from timestamp $ t = T $, compute the noise at the current level
\item Denoise the image completely
\item Reinject noise at rate $ \alpha_{t-1} $
\item Repeat untill $ t = 1 $
\end{itemize}
