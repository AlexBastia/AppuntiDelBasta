...
\end{itemize}
Che sono equivalenti. Questo ci da' l'apertura per un attacco a due parti:
\begin{enumerate}
  \item Creo una tablella che per ogni riga ha la tupla $ (k_i, E(k_i, m)) $ e la ordino per $ k $
  \item Per ogni $ k $, testa se $ D(k,c) $ sta nella seconda colonna. Se c'e', allora
    \[
...
\]
Dato che dobbiamo costruire e ordinare la tabella, poi cercare all'interno della tabella. DA STUDIARE LA METTE SEMPRE STA ROBA IN ESAME!!!!
...
\]
E' stra lento (3 operaizoni). Se $ k_1=k_2=k_3 $ ci troviamo nel caso di DES.

Ci sta sempre il meet in the middle attack, quindi il costo e' di $ < 2^{118} $ (comunque non poco).
...
\]
Facile da attaccare in tempo $ 2^{120} $


\subsection{Attacks on block ciphers}
Differential e' difficile su DES, ma si puo' usare il linear cryptanalysis dato che e' una cifratura iterativa. o

\subsection{AES}
DES non e' troppo efficente
