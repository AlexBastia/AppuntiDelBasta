...
\section{Linguaggio Universale e Problema della Fermata}
...
\pf{Dimostrazione}{
  Supponiamo per assurdo che esista $ M_{h\varepsilon}^* $ che decide $ HALT_\varepsilon $. Costruiamo $ \tilde{M}_w $: una macchina che ignora il proprio input, scrive $ w $ sul nastro e simula $ M $ su $ w $. Costruiamo quindi $ M' $ su input $ (M, w) $:
  \begin{enumerate}
    \item Costruisce $ \tilde{M}_w $ a partire da $ M $ e $ w $
...
\subsection{Teorema sulle riduzioni}
...
\subsection{Esempio: $ \mathcal{L}_{ne} $}
