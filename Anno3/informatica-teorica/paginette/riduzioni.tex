...
\item Come facciamo a trasformare istanze si del problema di partenza in istanze si del problema di arrivo e viceversa?
\end{enumerate}
...
$\overline{\mathrm{HALT}_\eps}$ è il suo complementare (le macchine che \emph{non} si fermano su~$\varepsilon$).
 
Poiché $\overline{\mathrm{HALT}_\eps} \notin \mathrm{RE}$, dalla riduzione segue che
...
\begin{itemize}
  \item Se $M$ \textbf{non si ferma} su $\varepsilon$ (cioè $M \notin \mathrm{HALT}_\epsilon$),
      ...
      \item Se $M$ \textbf{si ferma} su $\varepsilon$ (cioè $M \in \mathrm{HALT}_\eps$),
...
\thm{}{
  $M \notin \mathrm{HALT}_\eps \;\Longleftrightarrow\; g(M) \in \mathrm{Tiling}$.
}
\pf{Dimostrazione}{
...
codifica una computazione infinita $c_1 \vdash c_2 \vdash \cdots$, il che implica
  $M \notin \mathrm{HALT}_\eps$.
 
  Per contrapposizione: $M \in \mathrm{HALT}_\eps \Rightarrow g(M) \notin \mathrm{Tiling}$.
}
...
\begin{tikzpicture}[scale=1]
  ...
  \node[font=\small] at (-2.5, 0.6) {$\mathrm{HALT}_\eps$};
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\small] at (-2.5, 0.5) {$\mathrm{HALT}_\eps$};
  ...
  \node[font=\small, right=0.05cm] at (1,-0.1) {$\mathrm{HALT}_\eps$};
  % RE arch
...
\pf{Dimostrazione}{
  $\exists L \in \mathrm{RE}$ con $L \notin \mathrm{R}$: ad esempio $\mathrm{HALT}_\eps \in \mathrm{RE} \setminus \mathrm{R}$.
}
...
\subsection{Riduzione 1: \texorpdfstring{$\mathrm{HALT}_\eps \leq \mathrm{HALT}_{\forall}$}{HALT\_eps <= HALT\_forall}}
 
\textbf{Conseguenza:} $\mathrm{HALT}_\eps \notin \mathrm{coRE}$ e $\mathrm{HALT}_\eps \leq \mathrm{HALT}_{\forall}$, quindi
...
Quindi: $M \in \mathrm{HALT}_\eps \Leftrightarrow N \in \mathrm{HALT}_{\forall}$.
 
\subsection{Riduzione 2: \texorpdfstring{$\overline{\mathrm{HALT}_\eps} \leq \mathrm{HALT}_{\forall}$}{HALT\_eps-bar <= HALT\_forall}}
 
\textbf{Conseguenza:} $\overline{\mathrm{HALT}_\eps} \notin \mathrm{RE}$, quindi $\mathrm{HALT}_{\forall} \notin \mathrm{RE}$.
...
Quindi: $M \notin \mathrm{HALT}_\eps \Leftrightarrow N \in \mathrm{HALT}_{\forall}$, cioè
$\overline{\mathrm{HALT}_\eps} \leq \mathrm{HALT}_{\forall}$.
