...
\pf{Dimostrazione}{
  ...
  \end{enumerate}
  \begin{center}
    \includegraphics[width=0.6\textwidth]{img/costruzione-MA.png}
  \end{center}
...
\pf{Dimostrazione}{
  ...
  \end{enumerate}
  \begin{center}
    \includegraphics[width=0.6\textwidth]{img/costruzione-Mne.png}
  \end{center}
...
\pf{Dimostrazione}{
  ...
  \begin{center}
    \includegraphics[width=0.6\textwidth]{img/costruzione-Mprime.png}
  \end{center}
...
\dfn{Problema di Corrispondenza di Post (PCP)}{
  \textbf{Input}: lista di coppie di stringhe $ (r_i, s_i) $ con $ r_i, s_i \in \Sigma^* $ e $ |R| = |S| $.\\
  \textbf{Output}: accetta sse esiste una sequenza di indici $ i_1, \ldots, i_u $ con $ u > 0 $ tale che
  \[
    r_{i_1} \cdots r_{i_u} \;=\; s_{i_1} \cdots s_{i_u}.
  \]
}
...
\ex{}{
  ...
  \end{tabular}
  \end{center}
  la sequenza $ 2, 1, 1, 3 $ produce $ r = s = 101111110 $.
}

\thm{}{
  $ \mathrm{PCP} \in \mathrm{RE} $.
}
\pf{Dimostrazione}{
  Una MdT non deterministica indovina una sequenza di indici e verifica che le concatenazioni delle due colonne coincidano.
}

\dfn{Modified PCP (MPCP)}{
  Variante di PCP in cui ogni sequenza accettante deve iniziare dal primo indice ($ i_1 = 1 $).
}
...
\pf{Dimostrazione}{
  Mostriamo $ \mathcal{L}_u \leq \mathrm{MPCP} \leq \mathrm{PCP} $; poiche' $ \mathcal{L}_u \notin R $, segue $ \mathrm{PCP} \notin R $.

  \medskip
  \textbf{(1) $ \mathcal{L}_u \leq \mathrm{MPCP} $.}
  Idea: costruire un'istanza in cui un match coincide con la simulazione della storia di computazione accettante di $ M $ su $ w $. La colonna $ S $ scrive le configurazioni successive, $ R $ resta sempre una configurazione indietro; per pareggiare, $ M $ deve raggiungere uno stato accettante.

  Per la macchina
  \begin{center}
    \includegraphics[width=0.5\textwidth]{img/2026-03-16-19-11-14.png}
  \end{center}
  su $ w = 01 $ la computazione e'
  \[
    q_1 01 \vdash 1q_2 1 \vdash 10q_1 \vdash 1q_2 01 \vdash q_3 101.
  \]
  L'istanza MPCP costruita e':
  \begin{center}
    \begin{tabular}{c|c}
      $ R $ & $ S $ \\ \hline
      $ \# $ & $ \# q_1 01 \# $ \\
  ...
  \emph{Costruzione formale.} Sia $ q_0 $ lo stato iniziale e $ \Gamma $ l'alfabeto di nastro; le coppie sono raggruppate in sei famiglie:
  \begin{center}
    \begin{tabular}{ll|ll}
      &$ R $ & $ S $ & \\ \hline
      \emph{(1) inizio} &$ \# $ & $ \# q_0 w_1\ldots w_n\# $ & \\ \hline
      \emph{(2) copia} $ \forall x \in \Gamma $&$ x $ & $ x $ & \\
      &$ \# $ & $ \# $ &\\ \hline
      \emph{(3,4) transizioni} $ \forall q \in Q \setminus F $,&$ qx $ & $ yp $ & $ \delta(q,x) = (p, y, \to) $\\
      $ \forall x,y,z \in \Gamma $&$ zqx $ & $ pzy $ & $ \delta(q,x) = (p, y, \gets) $\\
      &$ q\# $ & $ yp\# $ & $ \delta(q,\cancel{b}) = (p, y, \to) $\\
      &$ zq\# $ & $ pzy\# $ & $ \delta(q,\cancel{b}) = (p, y, \gets) $\\ \hline
      \emph{(5) pulizia} $ \forall q \in F $,&$ xqy $&$ q  $&\\
      $ \forall x,y,z \in \Gamma $&$ xq $&$ q  $&\\
      &$ qy $&$ q  $&\\ \hline
      \emph{(6) chiusura} $ \forall q \in F $&$ q\#\# $&$ \#  $&
    \end{tabular}
  \end{center}
  Lettura delle famiglie:
  \begin{enumerate}
    \item \emph{Configurazione iniziale}: la prima riga forza l'inizio del match. $ R $ parte in svantaggio di lunghezza, e l'unico modo di colmarlo è simulare fedelmente $ M $.
    \item \emph{Copia invariata}: ogni simbolo del nastro non letto dalla testina viene ricopiato; analogamente il separatore $ \# $.
    \item \emph{Mosse a destra}: $ R $ legge $ qx $, $ S $ scrive $ yp $ secondo $ \delta $; le righe con $ q\# $ gestiscono la testina che esce a destra dalla configurazione corrente leggendo $ \cancel{b} $.
    \item \emph{Mosse a sinistra}: lo stato deve "scavalcare" il simbolo precedente $ z $, da cui $ zqx $ in $ R $ e $ pzy $ in $ S $.
    \item \emph{Pulizia}: raggiunto $ q \in F $, le tessere "mangiano" i simboli adiacenti, accorciando $ S $.
    \item \emph{Chiusura}: l'ultima tessera $ q\#\# $ contro $ \# $ pareggia le due stringhe.
  \end{enumerate}

  $ f $ è calcolabile: enumera un numero finito di regole guardando $ \delta $. Correttezza:
  \begin{itemize}
    \item $ (M,w) \in \mathcal{L}_u $: la sequenza di tessere che simula la computazione accettante di $ M $ su $ w $ produce $ r = s $; le regole (5)(6) chiudono il match.
    \item $ (M,w) \notin \mathcal{L}_u $: se $ M $ va in loop, $ S $ cresce di una configurazione per passo e $ R $ non potrà mai pareggiarla; se $ M $ rifiuta in $ q_{rej} \notin F $, le regole di pulizia non si attivano (nessuna riga di $ R $ contiene $ q_{rej} $ isolato) e nessuna sequenza chiude il match.
  \end{itemize}

  \medskip
  \textbf{(2) $ \mathrm{MPCP} \leq \mathrm{PCP} $.}
  Data un'istanza MPCP $ (R, S) $, costruiamo $ (T, U) $ inserendo un separatore $ * $ a destra di ogni carattere in $ R $ e a sinistra di ogni carattere in $ S $; aggiungiamo poi una tessera iniziale $ (t_0, u_0) = (*r_1,\, s_1) $ e una finale $ (t_{k+1}, u_{k+1}) = (\$,\, *\$) $:
  \begin{center}
    \begin{tabular}{c|c}
      $ R $ & $ S  $\\ \hline
      1 & 111\\
    ...
    \end{tabular}
  \end{center}
  $ t_0 $ è l'unica tessera in cui $ * $ compare in posizione iniziale, quindi forza $ i_1 = 0 $ (analogo del vincolo $ i_1 = 1 $ di MPCP); $ (\$, *\$) $ è l'unica con $ \$ $, quindi forza $ i_u = k+1 $.

  $ f $ è calcolabile. Correttezza:
  \begin{itemize}
    \item $ (R,S) \in \mathrm{MPCP} $ con sequenza $ 1, i_2, \ldots, i_n $ $ \Rightarrow $ $ (T,U) \in \mathrm{PCP} $ con sequenza $ 0, i_2, \ldots, i_n, k+1 $.
    \item $ (T,U) \in \mathrm{PCP} $: la sequenza inizia con $ 0 $ e termina con $ k+1 $; rimuovendo gli estremi e leggendo l'indice $ 0 $ come $ 1 $ si ottiene una soluzione MPCP per $ (R,S) $.
  \end{itemize}
}
...
colorati. Una piastrella speciale $d_0$ deve essere posizionata nell'origine
$(0,0)$.
...
\begin{tikzpicture}
  ...
  \draw[thick] (0,0) -- (1,1);
  \draw[thick] (1,0) -- (0,1);
  \node at (0.50,0.75) {\small $A$};
  \node at (0.75,0.50) {\small $B$};
  \node at (0.50,0.25) {\small $C$};
  \node at (0.25,0.50) {\small $D$};
...
Esibiamo un linguaggio fuori sia da RE che da coRE.

\dfn{$ \mathrm{HALT}_\forall $}{
  \[
...
\pf{Dimostrazione}{
  Mostriamo $ \mathrm{HALT}_\varepsilon \leq \mathrm{HALT}_{\forall} $: poiche' $ \mathrm{HALT}_\varepsilon \notin \mathrm{coRE} $, segue il risultato.

  Data $ M \in \mathrm{HALT}_\varepsilon $, dobbiamo costruire $ N = f(M) \in \mathrm{HALT}_{\forall} $ (e viceversa, $ M \notin \mathrm{HALT}_\varepsilon \Rightarrow N \notin \mathrm{HALT}_{\forall} $). Definiamo $ N $ che su input $ w $ ignora $ w $ e simula $ M $ su $ \varepsilon $.
  \begin{center}
  \begin{tikzpicture}[
      color=black,
      line width=0.8pt,
  ...
  \end{tikzpicture}
  \end{center}
  $ N $ si ferma su \emph{ogni} $ w $ se e solo se $ M $ si ferma su $ \varepsilon $, dunque
  \[
    M \in \mathrm{HALT}_\varepsilon \;\Leftrightarrow\; N \in \mathrm{HALT}_{\forall}.
  \]
}
...
\pf{Dimostrazione}{
  Mostriamo $ \overline{\mathrm{HALT}_\varepsilon} \leq \mathrm{HALT}_{\forall} $: poiche' $ \overline{\mathrm{HALT}_\varepsilon} \notin \mathrm{RE} $, segue il risultato.

  Data $ M \in \overline{\mathrm{HALT}_\varepsilon} $ (cioè $ M $ non si arresta su $ \varepsilon $), dobbiamo costruire $ N = f(M) \in \mathrm{HALT}_{\forall} $ (e viceversa, $ M \in \mathrm{HALT}_\varepsilon \Rightarrow N \notin \mathrm{HALT}_{\forall} $). Definiamo $ N $ che su input $ w $ simula $ M $ su $ \varepsilon $ per $ |w| $ passi (usando $ |w | $ come "countdown"); se la simulazione termina entro $ |w| $ passi $ N $ va in loop, altrimenti $ N $ si arresta.
  \begin{center}
  \begin{tikzpicture}[
      font=\sffamily,
      ...
      \draw[arrow] ([xshift=-1.5cm]outer.west) -- (outer.west) node[midway, above] {$w$};
      \draw[arrow] ([xshift=-1.2cm, yshift=0.5cm]inner.west) -- ([yshift=0.5cm]inner.west) node[midway, above] {$\varepsilon$};
      \draw[arrow] ([xshift=-1.2cm, yshift=-0.5cm]inner.west) -- ([yshift=-0.5cm]inner.west) node[midway, below] {$|w|$};
      \draw[arrow] ([yshift=0.5cm]inner.east) -- ([yshift=0.5cm]outer.east) node[midway, above] {no};
      \draw[arrow] ([yshift=0.5cm]outer.east) -- ++(1.5, 0) node[midway, above] {sí / no};
      \draw[arrow] ([yshift=-0.5cm]inner.east) -- ++(1, 0) node[midway, above] {si};
      \draw[arrow] ([yshift=-0.5cm, xshift=1cm]inner.east) -- ++(1, -0.8) node[right] {loop};
  \end{tikzpicture}
  \end{center}
  Internamente $ M_t $ su $ (\varepsilon, |w|) $ accetta sse $ M $ termina su $ \varepsilon $ entro $ |w| $ passi (e termina sempre, per qualsiasi $ M $).

  Correttezza:
  \begin{itemize}
    \item $ M \notin \mathrm{HALT}_\varepsilon $: la simulazione non termina mai entro $ |w| $ passi per nessun $ w $, quindi $ N $ si arresta sempre $ \Rightarrow N \in \mathrm{HALT}_{\forall} $.
    \item $ M \in \mathrm{HALT}_\varepsilon $ in $ k $ passi: per ogni $ w $ con $ |w| \geq k $, $ M_t $ accetta e $ N $ va in loop $ \Rightarrow N \notin \mathrm{HALT}_{\forall} $.
  \end{itemize}
  Dunque $ M \notin \mathrm{HALT}_\varepsilon \Leftrightarrow N \in \mathrm{HALT}_{\forall} $.
}
...
\pf{Dimostrazione}{
Sia $ P \subseteq \mathrm{RE} $ non banale. Per ricordo,
\[
  L_P = \{\, \langle M \rangle \mid \mathcal{L}(M) \in P \,\}.
\]
Distinguiamo due casi a seconda che $ \emptyset \in P $ o meno.

\medskip
\textbf{Caso (a): $ \emptyset \notin P $.}

Poiché $ P $ non è banale ed $ \emptyset \notin P $, esiste $ L \in P $ con $ L \neq \emptyset $; sia $ M_L $ tale che $ \mathcal{L}(M_L) = L $.

Mostriamo $ L_u \leq L_P $. Data $ (M, w) \in L_u $, dobbiamo costruire $ N = f(M, w) \in L_P $ (e viceversa, $ (M,w) \notin L_u \Rightarrow N \notin L_P $). Definiamo $ N $ che su input $ x $:
\begin{enumerate}
  \item simula $ M $ su $ w $;
  \item se $ M $ accetta $ w $, esegue $ M_L $ su $ x $ e ne restituisce il risultato.
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    thick,
    >=stealth,
    box/.style={draw=black, thick, minimum size=1.5cm, align=center}
]
    ...
    \draw[->] ($(N.west |- M.west) + (0.8cm, 0)$) -- (M.west) node[midway, above] {$w$};
    \draw[->] ($(N.west |- Mc.west)$) -- (Mc.west) node[midway, above, near start] {$x$};
...
Correttezza:
\begin{itemize}
  \item[$ (\Rightarrow) $] $ (M,w) \in L_u $: $ N $ si comporta come $ M_L $ su ogni $ x $, quindi $ \mathcal{L}(N) = \mathcal{L}(M_L) = L $. Per la semanticità di $ P $, $ \mathcal{L}(N) = L \in P \Rightarrow N \in L_P $.
  \item[$ (\Leftarrow) $] $ (M,w) \notin L_u $: $ N $ non supera mai il passo 1, quindi $ \mathcal{L}(N) = \emptyset $. Poiché $ \emptyset \notin P $, $ N \notin L_P $.
\end{itemize}
$ f $ è calcolabile (manipola descrizioni di MdT). Quindi $ L_u \leq L_P $ e $ L_P \notin R $.

\medskip
\textbf{Caso (b): $ \emptyset \in P $.}

Consideriamo $ \overline{P} = \mathrm{RE} \setminus P $. Allora $ \emptyset \notin \overline{P} $, $ \overline{P} $ non è banale ed è semantica (eredita la proprietà da $ P $). Per il Caso (a), $ L_{\overline{P}} \notin R $. Poiché $ L_P = \overline{L_{\overline{P}}} $ e $ R $ è chiusa per complemento, $ L_P \notin R $.
}
