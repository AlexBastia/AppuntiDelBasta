...
\begin{itemize}
  ...
  \item se $rr(A)=rr(A|b)=k,k < n \implies$ esistono infinite soluzioni che dipendono da n-k parametri
\end{itemize}
...
\end{pmatrix}
        \]
        $rr(A)=rr(A|b)=1<n=2$, quindi esistono infinite soluzioni dipendenti da 2-1=1 parametri.
  \end{enumerate}
}
...
\dfn{Operazioni elementari}{
  ...
  \item Scambio di due righe $R_i \leftrightarrow R_j$.
    \item Moltiplicazione di una riga per $c\in\mathbb{R}, c\neq0$ $R_i \to cR_i$.
...
\nt{
  La matrice a scala non e' univocamente determinata. Quindi partendo da una stessa matrice possiamo ottenere diverse matrici a scala equivalenti.
}
...
\ex{Spazio vettoriale}{
  ...
  \item Sappiamo che $ \mathbb{R}^2 $ e' chiuso rispetto alla somma e al prodotto per uno scalare.
  \end{itemize}
...
\dfn{Sottospazio}{
  ...
  \item $ \forall \lambda \in \mathbb{R},\forall v \in U. \lambda v \in U $(chiusura rispetto al prodotto per scalari)
  \end{enumerate}
}
...
\nt{
  ...
  \begin{enumerate}
    \item $ U = \{\underline{0}\} $ (sottospazio banale),
...
\mprop{}{ \label{siSV}
  Sia V SV e $ \{v_1,...,v_n\} \subseteq V $, allora:
  \[ <v_1,...,v_n> \leq V \]
  Inoltre, se $ Z \leq V \land \{v_1,...,v_n\} \subseteq Z $, allora:
  \[ <v_1,...,v_n> \subseteq Z \] 
}
Cio' significa che l'insieme di combinazioni di \textbf{qualunque} sotto-insieme di vettori di uno spazio vettoriale e' un sottospazio vettoriale. Inoltre tale sotto-spazio e' il piu' piccolo contenente i vettori del sotto-insieme.
\dfn{Generatore}{
...
\cor{}{
  \textbf{Due} vettori sono lin dip $ \iff $ uno di essi e' multiplo dell'altro
}
\ex{}{
...
\mprop{}{
  L'insieme di soluzioni di un sistema lineare omogeneo e' sempre un sottospazio.
}
\ex{}{
  $ \begin{pmatrix}
  0 & 3 & 0\\
...
\dfn{Nucleo e Immagine}{
  $ F: V\to W $ lin
  \[
    kerF=\{v\in V | F(v)=\underline{0}\} \subseteq V
  \]
  \[
...
\ex{}{
  ...
  \begin{itemize}
    ...
    \item $ F(0,0,1) = (0,-1) $
  \end{itemize}
  Dato che $ \{(1,0,0),(0,1,0),(0,0,1)\} $ e' base (canonica) di $ \mathbb{R}^3 $, e dato che conosco $ F(v) $ per ogni vettore di tale base, allora posso trovare $ F(v) $ per ogni $ v \in \mathbb{R}^3 $ usando la linearita':
  \[
...
\pf{}{
  Sia $ A \in M_{m\times n}(\mathbb{R}) $ e sia $ L_A: V\to W $ l'applicazione lineare associata. Il rango colonne di A conincide con la dimensione dell' immagine di $ L_A $. La dimensione del nucleo e' data dal numero di colonne di A (n) meno il rango righe. Quindi $ dim(KerL_A) = n - rr(A) $, allora $ rr(A) = n - dim(KerL_A) $. Dobbiamo dimostrare che $ rr(A) = rc(A) $, quindi che $ n - dim(KerL_A) = dim(ImL_A) $. Dato che le colonne sono l'immagine della base di $ V $ rispetto a $ L_A $, possiamo dire che $ n = dim(V) $. Quindi dobbiamo dimostrare che $ dim(V) = dim(ImL_A) + dim(KerL_A) $, ovvio per teo. dim.
}
\section{Isomorfismo}
\dfn{Isomorfismo}{
...
\pf{}{
  ...
  \begin{itemize}
    ...
    \item $ \impliedby) $ Sapendo che $ dimV = dimW = n $, possiamo dire che esistono la base $ \{v_1,...,v_n\} $ di $ V $ e la base $ \{u_1,...,u_n\} $ di W che hanno lo stesso numero di elementi. Prendiamo l'applicazione lineare $ F:V\to W $ per cui $ F(u_i) = v_i $ e dimostriamo che e' biettiva. Sappiamo che $ ImF = <v_1,...,v_n> $, ma anche $ W $ e' generato dagli stessi vettori, quindi $ ImF = W $ ed $ F $ e' suriettiva. Per il teo. dim. abbiamo che $ dim(KerF) = dimV - dim(ImF) $, ma noi sappiamo che $ dimV = dimW $, e che $ dimW - dim(ImF) = 0 $. Quindi $ dim(KerF) = 0 $ che implica che $ F $ e' anche iniettiva.  
  \end{itemize}
}
...
\mprop{Applicazioni lineari composte}{
  Siano $ F:O\to W,G:V\to O $ app. lin., allora la funzione composta:
  \[
  F\circ G:V\to W \text{ e' lineare}
  \]
}
\cor{}{
...
\ex{}{
  Data un'applicazione lineare $ L_A: \mathbb{R}^2\to\mathbb{R} $ con matrice associata $ A = \begin{pmatrix}
  2 & -2\\
  1 & -1\\
  \end{pmatrix} $, abbiamo che $ ImmL_A = \generators{(2,1)} $ e $ KerL_A = \generators{(1,1)} $:
  \begin{center}
    \includegraphics[width=0.5\textwidth]{img/2024-05-11-11-41-43.png}
  \end{center}
  Prendiamo un vettore $ v = (1,0) $ e calcoliamo la sua immagine $ u = (2,1) $. Ora calcoliamo la controimmagine del vettore $ u $ appena trovato usando la proposizione sopra: $ L_A^{-1}(u) = \{v+z | z \in KerL_A\} $, che graficamente e' la retta passante per la punta di $ v $ con la stessa pendenza della retta formata da $ KerL_A $:
  \begin{center}
    \includegraphics[width=0.5\textwidth]{img/2024-05-11-11-51-07.png}
  \end{center}
}
...
\thm{Rouche'-Capelli}{
  Un sistema lineare $ Ax=b $ di m equazioni in n variabili ha soluzioni sse $ rk(A) = rk(A|b) $, in tale caso si ha:
  \begin{itemize}
    \item Se $ rk(A) = rk(A|b) = n $ allora esiste una sola soluzione
    \item Se $ rk(A) = rk(A|b) < n $ allora ci sono infinite soluzioni che dipendono da $ n - rk(A) $ parametri.
  \end{itemize}
}
\pf{}{
  Dimostriamo che un sistema lineare $ Ax = b $ ha soluzioni sse rk(A) = rk(A|b). Dimostriamo entrambe le direzioni:
  \begin{itemize}
    \item $ \implies) $ Assumiamo che il sistema abbia soluzioni. Sappiamo quindi che $ b \in ImL_A $, ovvero che $ b $ appartiene al sottospazio generato dalle colonne di $ A $. Cio' implica che $ dim\generators{\text{colonne di }A} = \generators{\text{colonne di }A|b} $, quindi per \ref{colEqRig} $ rk(A) = rk(A|b) $.
...
\subsection{Definizione e proprieta'}
Il determinante di una matrice quadrata e' un numero reale che e' importante per due motivi:
\begin{itemize}
\item A e' invertibile $ \iff $ $ detA \neq 0 $
\item Se consideriamo $ L_A: \mathbb{R}^2\to\mathbb{R}^2 $ o $ \mathbb{R}^3\to\mathbb{R}^3 $, $ detA $ e' il fattore che determina come cambiano le aree o i volumi quando applichiamo $ L_A $.
\end{itemize}
...
Dalla definizione derivano altre proprieta' importanti del determinante:
\mprop{}{
  Siano $ A,B $ due matrici quadrate di ordine $ n $:
  \begin{itemize}
  \item Se $ B $ e' ottenuta da $ A $ scambiando due righe, allora:
    \[
      det(B) = -det(A)
    \]
    \item Se $ B $ e' ottenuta da $ A $ sommando ad una riga di $ A $ una quaunque combinazione lineare delle altre righe, allora:
      \[
        det(B) = det(A)
      \]
  \end{itemize}
...
\nt{
  Tutte queste proprieta' valgono anche per le colonne (pensio sia perche' $ detA = detA^T $ ? non so neanche se e' vero).
}
\thm{Binet}{
...
\mprop{}{
  Data una matrice triangolare $ A $ di ordine $ n $, il suo determinante e' dato da:
  \[
...
\ex{}{
  ...
  \[
    det(A) = -det(A') = 7
  \]
  (Il meno e' perche' abbiamo scambiato la seconda e la terza riga)
}
...
\dfn{Minore di una matrice}{
  Data una matrice quadrata $ A $ di ordine $ n $, si indica con $ A_{ij} $ la sottomatrice ottenuta cancellando da $ A $ la riga $ i $ e la colonna $ j $. Allora $ A_{ij} $ si dice \textbf{minore} di $ A $ di ordine $ n-1 $.
}
  ...
  \begin{itemize}
    \item Se $ A $ ha ordine 1, cioe' $ A = (a_{11}) $ ha una riga e una colonna, allora:
      \[
        det(A) = a_{11}
      \]
    \item Se $ A $ ha ordine $ > 1 $, sia:
      \[
        \Gamma_{ij} = (-1)^{i+j}detA_{ij}
      \]
      Allora lo sviluppo di $ detA $ secondo la \textit{r-esima} riga e':
      \[
      detA = \sum_{k=1}^{n}a_{rk}\Gamma_{rk} 
      \]
      Mentre lo sviluppo secondo la \textit{s-esima} colonna e':
      \[
      detA = \sum_{k=1}^{n} a_{ks}\Gamma_{ks}
      \]
  \end{itemize}
}
Come avevo scritto prima, conviene scegliere la riga/colonna che contiene piu' zeri, dato che ci permettono di non calcolare il $ \Gamma $ di quella sottomatrice. Inoltre, dato che il calcolo del determinante di una matrice di ordine $ 2 $ e' semplice, conviene prendere tali matrici come caso base per velocizzare l'algoritmo.

\section{Invertibilita'}

\dfn{Matrice invertibile}{
  Una matrice quadrata $ A $ si dice \textbf{invertibile} se esiste una matrice quadrata $ B $ dello stesso ordine $ n $ t.c.
  \[
  AB = BA = I_n
  \]
}
...
\pf{}{
  ...
  \begin{itemize}
    ...
    \item $ \impliedby ) $ Se il $ det(A) \neq 0 $, allora si puo' dimostrare che e' invertibile e che la sua matrice inversa $ A^{-1} $ e' data da questa formula:
      \[
        (A^{-1})_{ij} = \frac{(-1)^{i+j}}{det(A)}det(A_{ji})
      \]
      Questo e' il metodo di Laplace per trovare l'inversa di una matrice.
  \end{itemize}
}

Un altro metodo per calcolare la matrice inversa e' l'algoritmo di GAUSS:
\begin{itemize}
...
\nt{
  Se A e' una matrice quadrata e B t.c. AB = I, allora BA = I. Quindi non c'e' bisogno di controllare entrambe le direzioni. ($ AB = I \implies (A^{-1}A)B = A^{-1}I \implies IB = A^{-1}I \implies B = A^{-1} \implies BA = I $)
}
\section{Riassuntino}
\mprop{Teoremone}{
  Sia $ F: \mathbb{R}^n\to\mathbb{R}^n $, allora sono equivalenti:
  \begin{itemize}
    \item F e' un isomorfismo (invertibile)
  \item F e' iniettiva
  \item F e' suriettiva
  \item dim(ImF) = n
  \item rk(A) = n
...
\mprop{}{
  sia $ F\colon\mathbb{R}^n\to\mathbb{R}^n $, con autovalore $ 0 $:
  \[
    kerF = V_0 \neq \{\underline{0}\}
  \]
  Quindi $ dim(kerF) = dim(V_0) \geq 1 $ e $ F $ non e' invertibile!
}
\pf{}{
  Se un isomorfismo $ F:\mathbb{R}^n\to\mathbb{R}^n $ ha un autovalore $ \lambda = 0 $, significa che esiste un autospazio $ V_0 $ t.c. $ dimV_0 \geq 1 $ per cui $ \forall v \in V_0. F(v) = \underline{0} $. Per definizione, l'autospazio $ V_0 $ coincide con il nucleo di $ K $, quindi $ kerF = V_0 $ e $ dim(KerF) \geq 1 $, quindi $ F $ non e' invertibile.
}
\nt{
...
\section{Vettori e basi ortogonali}
...
\pf{}{
  La $ dimW^\perp $ e' l'insieme delle soluzioni del sistema rappresentato dalla matrice omogenea che ha come righe la base di $ W $, che quindi ha dimensione $ n - r(W^\perp) $. Ma noi sappiamo che il rango righe e il rango colonne di una matrice coincidono, quindi $ r(W^\perp) = r(W) = k = dimW $. Allora per il teorema della dimensione $ dimW^\perp = n - dimW $. \\
  Per dimostrare la seconda parte, e' subito ovvio che dato un vettore $ v \in W^\perp $ per cui $ \forall w \in W. \innerproduct{v}{w} = 0 $ puo' appartenere anche a $ W $ solo se $ v = \underline{0} $.
}
\section{Proiezione ortogonale}  
\dfn{Proiezione ortogonale}{
  Dati due vettori $ v,w \in \mathbb{R}^n $, si dice \textbf{proiezione di v su w} il vettore $ \overline{v} $ dato da:
  \[
    proj_w(v) = \overline{v} = \frac{\innerproduct{v}{w}}{\innerproduct{w}{w}}w
  \]
}
Inoltre, se noi togliamo dal vettore $ v $ la sua proiezione su $ w $, allora rimane solo la componente di $ v $ ortogonale a $ w $, quindi in generale:
\mprop{}{
  Dati due vettori $ v,w \in \mathbb{R}^n $, il vettore $ \overline{v} $ dato da:
  \[
    \overline{v} = v - proj_w(v)
  \]
  E' ortogonale rispetto a $ w $.
}

\section{Algoritmo di Gram-Schmidt}
Algoritmo per trovare una base ortonormale di un sottospazio data una sua qualunque base. Utilizza la proposizione precedente per trovare la componente di ogni vettore che e' perpendicolare a tutti i vettori precedenti, formalmente:
\mprop{}{
  Dato un SV $ W $ e una sua base $ \{v_1,...,v_k\} $, e' possibile trovare una base ortonormale di $ W $ seguendo questo procedimento:
  \begin{itemize}
  \item Prima di tutto troviamo una base ortogonale:
    \[
    w_i = \begin{cases}
    w_1 & i = 1\\
      v_i - proj_{v_1}(v_i) - ... - proj_{v_{i-1}}(v_i) & i \neq 1
    \end{cases}
    \]
    \item Ora basta normalizzare i vettori $ w_1,...,w_k $ per ottenere una base ortonormale:
      \[
      f_i = \frac{w_i}{||w_i||}
      \]
  \end{itemize}
  Quindi $ \{f_1,...,f_k\} $ e' una base ortonormale di $ W $.
}


\section{Applicazioni e matrici ortogonali}
\dfn{}{
  Una app. lin. $ F:\mathbb{R}^n\to\mathbb{R}^n $ si dice ortogonale se:
  \[
   \forall u,v \in \mathbb{R}^n. \innerproduct{F(u)}{F(v)} = \innerproduct{u}{v}
  \]
}
Quindi $ F $ e' una applicazione lineare che conserva le lunghezze $ (||F(v)|| = \sqrt{\innerproduct{F(v)}{F(v)}} = \sqrt{\innerproduct{v}{v}} = ||v||) $ e che conserva gli angoli $ (\innerproduct{F(v)}{F(w)} = ||F(v)||||F(w)||cos\theta = ||v||||w||cos\gamma \implies cos\theta = cos\gamma) $.
\dfn{Matrice ortogonale}{
  Una matrice $ A \in M_n(\mathbb{R}) $ si dice ortogonale se:
  \[
    \forall u, v \in \mathbb{R}^n. \innerproduct{Au}{Av} = \innerproduct{u}{v}
  \]
}
E' ovvio per definizione che:
\mprop{}{
  Una applicazione lineare $ L_A $ e' ortogonale $ \iff $ la sua matrice associata $ A $ e' ortogonale
}
Vediamo delle implicazioni interessanti riguardo le matrici ortogonali: 
\mprop{}{
  Data una matrice quadrata $ A \in M_n(\mathbb{R}) $, sono equivalenti:
  \begin{itemize}
  \item $ A $ e' ortogonale
  \item $ \forall x \in \mathbb{R}^n. <Ax,Ax> = <x,x> $
  \item $ A A^T = I $, cioe' $ A^{-1} = A^T $
  \item Le colonne e le righe di $ A $ formano due basi ortogonali di $ \mathbb{R}^n $
  \end{itemize}
}
\pf{}{
  $ 3 \iff 4 $
...
\section{Simmetria}
\dfn{Endomorfismo simmetrico}{
  Dato un endomorfismo $ F:\mathbb{R}^n\to\mathbb{R}^n $, si dice simmetrico se:
  \[
    \forall u,v \in \mathbb{R}^n. \innerproduct{F(u)}{v} = \innerproduct{u}{F(v)}
  \]  
}
\dfn{Matrice simmetrica}{
  Una matrice quadrata $ A \in M_n(\mathbb{R}) $ si dice simmetrica se:
  \[
  \forall i,j = 1,...,n. c_{ij} = c_{ji}
  \]
}
\mprop{}{
  Sia $ F:\mathbb{R}^n\to\mathbb{R}^n $ un endomorfismo e sia $ A = M_\mathcal{C}^\mathcal{C}(F) $:
  \[
  F \text{ e' simmetrica } \iff A \text{ e' simmetrica}
  \]
}
\section{Teorema spettrale}
\thm{Spettrale}{
  Sia $ F:\mathbb{R}^n\to\mathbb{R}^n $ un endomorfismo simmetrico, allora esiste una base di $ \mathbb{R}^n $ costituita da autovettori di $ F $. Piu' in particolare:
  \begin{itemize}
    \item $ F $ e' diagonalizzabile
  \item Se $ \lambda_1, \lambda_2 $ sono due autovalori distinti, allora gli autospazi relativi $ V_{\lambda_1}, V_{\lambda_2} $ sono ortogonali
  \item Se $ A = M_\mathcal{C}^\mathcal{C}(F) $, allora esiste una matrice ortogonale $ P $ per cui esiste una matrice diagonale $ D $ tale che $ D = P^{-1}AP = P^TAP $. Ovvero, $ A $ e' "ortogonalmente simile" a una matrice diagonale.
  \end{itemize}
}

\chapter{Divisione Euclidea}
Dati $ a,b \in \mathbb{N}, b\neq 0 $, $ \exists! q,m \in \mathbb{N} $ tali che $ a = bq+r $ dove q e' il \textbf{quoziente} e $ 0 \leq r < b $ e' il \textbf{resto} sono unici.

  Dati $ a,b \in \mathbb{Z} $ diciamo che b divide $ a $ e scriviamo $ b|a $ se $ \exists c \in \mathbb{Z} $ tale che $ a = bc $

\nt{
  $ b|0 $ perche' $ 0=b 0 $
}
\thm{Bezout}{
  Dati $ a,b \in \mathbb{N} $ (vale anche con $ a,b \in \mathbb{Z} $), $ \exists r,s \in \mathbb{Z} $ tali che:
  \[
    mcd(a,b) = ra + sb
  \]
}
\nt{
  $ r $ e $ s $ non sono unici
}
\subsection{Relazione di congruenza}
Dati $ a,b \in \mathbb{Z} $, $ n \in \mathbb{N}, n\neq 0 $ diciamo che $ a $ e' congruo a $ b $ modulo $ n $:
\[
a congruent _n b se n|a-b
\]
