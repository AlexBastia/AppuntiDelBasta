...
\end{itemize}
\subsection{linguaggi liberi deterministici}
\definizione{
    Un linguaggio è \textbf{liebro deterministico} se è accettato per stato finale da un DPDA
}
\teorema{
...
\esempio{
    \begin{itemize}
        ...
        \begin{tikzpicture}
            ...
            % Extra circles around states
            \draw[thick, dashed] (q0) circle[radius=1cm];
            \draw[thick, dashed] (q1) circle[radius=1cm];
            \draw[thick, dashed] (q2) circle[radius=1cm];
            \draw[thick, dashed] (q3) circle[radius=1cm];
...
Si giunge così alla seguente osservazione:
\nt{
...
\begin{itemize}
    \item \textb{Chomsky}: Una grammatica è in forma normale di Chomsky se ogni produzione ha la forma $A\to BC$ o $A\to a$, dove $A, B,C\in NT$ e $a\in T$. Ogni produzione deriva quindi o una coppia di variabili o un singolo terminale. Questa forma è utile, per esempio, negli algoritmi di parsing
