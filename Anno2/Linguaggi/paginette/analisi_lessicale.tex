...
\esempio{
    ...
    Prima di tuo costruiamo una tabella con solo delle coppie che non sono identiche per far ciò la costruiamo a scaletta. Si ha che:

    %TODO disegnare la tabella bella
}
...
\dimostrazione{
    Procedo a dimostrare le due parti parte 
    \begin{itemize}
        ...
        \begin{itemize}
            \item $\Rightarrow$
            Se $p$ e $q$ sono distinguibili, allora $\exists x\in \Sigma^*.\hat{\delta}(p,x)\in F$ e $\hat{\deltaq}(p,x \notin F)$ (o viceversa).
