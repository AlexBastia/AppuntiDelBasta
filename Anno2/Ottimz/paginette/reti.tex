...
Data una rete $G$, un flusso ammissibile $x$ e due nodi $,j$ in $G$, indiachiamo con $\delta_x (i,j)$ la distanza tra $i$ e $j$ nel grafo residuo $G_x$
\mlenma{}{
...
\nt{
  In ongi cammino aumentante, esiste almeno un arco critico
}
...
\dimostrazione{
  ...
  \begin{itemize}
    ...
    \]
    Cioè, $j$ si trova a distanza 1 in più di $i$ dalla sorgente nel BFS.
...
Un'altro approccio possibile che permette di scendere sotto la soglia di una complessità $O(V\,A^2)$ consiste nel rendere la costruzione del flusso massimo più locale    (in opposizione con FF ed EK, in cui ad ogni iterazione occorre procedere con un’analisi globale), un concetto fondamentale che sta dietro all'algoritmo è il concetto di \textbf{preflusso}
...
\thm{struttura degli Psudoflussi}{
    Siano x e y due pseudoflussi qualunque. Allora esistono k ≤ n + m cammini aumentanti P1 , . . . , Pk , tutti per x, di cui al più m sono cicli, tali che:
    \begin{equation}
      \begin{aligned}
        z1 = x\\
    z_{i+1} = zi ⊕ θi Pi\\
